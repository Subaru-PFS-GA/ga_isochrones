...
class Script():
    ...
    def add_subparsers(self, parser):
        # Default behavior doesn't use subparsers
    ...
    def add_args(self, parser):
        ...
        parser.add_argument('--tf-profile', action='store_true', help='Run in TensorFlow profiler mode\n')
    ...
    def parse_args(self):
        if self.args is None:
            ...
            if self.parse_unknown:
                ...
                self.args, self.unknown_args = self.parser.parse_args(), None
            self.args = self.args.__dict__
            ...
            if len(configs) > 0:
                ...
                if self.parse_unknown:
                    command_args, _ = self.parser.parse_known_args()
                else:
                    command_args = self.parser.parse_args()
                command_args = command_args.__dict__
            ...
            if 'skip_notebooks' in self.args and self.args['skip_notebooks'] is not None:
                self.skip_notebooks = self.args['skip_notebooks']
    ...
    @staticmethod
    def get_env_vars(prefix='ISOCHRONES'):
        vars = {}
        for k in os.environ:
